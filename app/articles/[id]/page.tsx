      ...
      <div style={{maxWidth:780,margin:'0 auto',padding:'7rem 1.5rem 4rem',position:'relative',zIndex:1}}>
        ...
        <div style={{display:'flex',gap:'.75rem',alignItems:'center',flexWrap:'wrap',padding:'1.25rem',background:T.card,border:`1px solid ${T.border}`,borderRadius:14,marginBottom:'2.5rem'}}>
          <button onClick={handleLike} disabled={liked}
            style={{display:'flex',alignItems:'center',gap:'.5rem',padding:'.65rem 1.25rem',background:liked?'rgba(208,112,128,0.2)':'rgba(138,31,50,0.2)',border:`1px solid ${liked?T.rose:T.border}`,borderRadius:10,color:liked?T.rose:T.text2,fontSize:'.9rem',fontWeight:700,cursor:liked?'default':'pointer'}}>
          ...
          {[['twitter','𝕏'],['linkedin','in'],['whatsapp','📱']].map(([p,icon])=>(
            <button key={p} onClick={()=>handleShare(p)}
              style={{padding:'.55rem 1rem',background:'rgba(255,255,255,0.04)',border:`1px solid ${T.border}`,borderRadius:8,color:T.text2,fontSize:'.88rem',fontWeight:700,cursor:'pointer'}}>
              {icon}
            </button>
          ))}
        </div>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:'.85rem'}}>
              ...
              <div style={{display:'flex',alignItems:'center',gap:'1rem',flexWrap:'wrap'}}>
                <button onClick={handleComment} disabled={posting||!name.trim()||!text.trim()}
                  style={{padding:'.7rem 1.5rem',background:`linear-gradient(135deg,${T.burgL},#4a0f1c)`,color:T.white,border:'none',borderRadius:10,fontWeight:700,fontSize:'.9rem',cursor:(posting||!name.trim()||!text.trim())?'not-allowed':'pointer',opacity:(posting||!name.trim()||!text.trim())?.6:1}}>
