...
export default function CoursesPage() {
  ...
  return (
    <>
      <style>{`
        .courses-hero { padding: 9rem 3rem 4rem; }
        .courses-content { max-width:1050px; margin:0 auto; padding:3rem 3rem 6rem; }
        .courses-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(280px,1fr)); gap:1.5rem; }
        .course-cover { width:100%; height:150px; overflow:hidden; border-radius:10px; margin-bottom:1.1rem; border:1px solid ${T.border}; }
        .course-cover img { width:100%; height:100%; object-fit:contain; display:block; background:#0e0608; }

        @media(max-width:768px){
          .courses-hero    { padding: 6.5rem 1.25rem 2.5rem; }
          .courses-content { padding: 1.5rem 1.25rem 4rem; }
      ...
      <main style={{background:T.bg,color:T.text,minHeight:'100vh',fontFamily:"'IBM Plex Sans Arabic','DM Sans',sans-serif",direction:L.dir}}>
        ...
        <section className="courses-hero" style={{position:'relative',overflow:'hidden',background:T.bg2}}>
          ...
          <div className="courses-grid">
            ...
            {courses.map((c,idx) => (
              <Link key={c.id} href={`/courses/${c.id}`}
                style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:16,padding:'1.5rem',textDecoration:'none',display:'flex',flexDirection:'column',transition:'all .3s'}}
                ...
                <div style={{fontSize:'.85rem',color:T.text2,lineHeight:1.9,flex:1,marginBottom:'1rem'}}>{c.desc}</div>
